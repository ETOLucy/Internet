angular.module('elastichat', ['ionic', 'monospaced.elastic', 'angularMoment'])
	...
	.controller('UserMessagesCtrl', ['$scope', '$rootScope', '$state', '$stateParams', 'MockService', '$ionicActionSheet', '$ionicPopup', '$ionicScrollDelegate', '$timeout', '$interval',
				...
				link: function (scope, element, attrs) {
					$timeout(function () {
						...
						for (var i = 0; i < autolinks.length; i++) {
							angular.element(autolinks[i]).bind('click',
								function (e) {
...
// 	this.ele.src = '../../statics/images/用户1.svg'; // set a fallback
// }
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function waitForElements() {
        // 尝试获取所有目标元素
        var displayImageElements = document.querySelectorAll('.replaceableImage');

        // 如果找到了目标元素
        if (displayImageElements.length > 0) {
            // 获取图像的 URL
            var imageUrl = localStorage.getItem('imageUrl');
			if(imageUrl) {
				// 设置所有图像元素的 src 属性
				displayImageElements.forEach(function(element) {
            ...
            // 如果未找到目标元素，则等待一段时间后再次尝试
            setTimeout(waitForElements, 100); // 100 毫秒后再次尝试
        }
    }
